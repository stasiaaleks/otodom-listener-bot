...
def parse_next_data(html: str) -> list[ListingDTO]:
...
def _to_listing(it: dict) -> ListingDTO:
    return ListingDTO(
        id=it["id"],
        ...
        created_at=_datetime(it.get("dateCreated") or it.get("createdAtFirst")),
...
def _money(d: dict | None) -> FinancialDTO | None:
...
def _datetime(s: str | None) -> datetime | None:
    if not s:
        return None
    s = s.replace("Z", "+00:00")
